...
const ChatPage: React.FC = () => {
    ...
    // REFACTORED: Combined socket connection and event listeners into one stable useEffect.
    useEffect(() => {
        if (!user) return;
        ...
        // 2. Define STABLE listeners that will not be re-registered
        const handleMessageReceived = (newMessage: Message) => {
            ...
            // Use the ref to check if the message belongs to the currently open chat
            if (selectedChatRef.current && newMessage.chat === selectedChatRef.current._id) {
        ...
        // 3. Register the stable listeners
        ...
        // This useEffect should only run when the user logs in or out.
    }, [user, logout, getChatName]);
